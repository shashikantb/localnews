...
const dowMatches = (dbDow: number, jsDow: number) => {
  ...
  if (dbDow === 7) return jsDow === 0; // Sun=7 style
  return dbDow - 1 === jsDow; // 1..7 Mon..Sun
};

// Normalize ids so Set membership works even if types differ
...
export default function BookingDialog({ business, sessionUser, children }: BookingDialogProps) {
  ...
  const handleCreateAppointment = async () => {
    ...
    const bookedResourceIds = new Set(appointmentsInSlot.map((a) => idStr(a.resource_id)));
    const availableResource = effectiveResources.find((r) => !bookedResourceIds.has(idStr(r.id)));
    ...
    const result = await api<{ success: boolean; error?: string; appointment?: any }>(`/api/booking/appointments`, {
      method: 'POST',
    ...
    if (result.success) {
      ...
      toast({
        ...
        description: result.error || 'Please try another slot.',
      });
    }
    setIsSubmitting(false);
  };
